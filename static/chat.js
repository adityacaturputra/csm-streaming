...
async function matchSavedDeviceIds(savedMicId, savedOutputId) {
  try {
    ...
    if (!foundMic && savedMicId) {
      const savedMicDevice = devices.find(
        (d) => d.deviceId === savedMicId && d.kind === "audioinput"
      );
      if (savedMicDevice && savedMicDevice.label) {
        const savedLabel = savedMicDevice.label;
        for (let i = 0; i < micSelect.options.length; i++) {
          const optDevice = devices.find(
            (d) => d.deviceId === micSelect.options[i].value
          );
    ...
    if (!foundOutput && savedOutputId) {
      ...
      const savedOutputDevice = devices.find(
        (d) => d.deviceId === savedOutputId && d.kind === "audiooutput"
      );
      if (savedOutputDevice && savedOutputDevice.label) {
        const savedLabel = savedOutputDevice.label;
        for (let i = 0; i < outputSelect.options.length; i++) {
          const optDevice = devices.find(
            (d) => d.deviceId === outputSelect.options[i].value
          );
          if (optDevice && optDevice.label === savedLabel) {
            outputSelect.selectedIndex = i;
...
function connectWebSocket() {
  ...
  ws.onopen = () => {
    ...
    addMessageToConversation(
      "ai",
      "WebSocket connected. Ready for voice input."
    );
  };
  ...
  if (data.config) {
    ...
    if (data.config.vad_threshold) {
      ...
      if (vadThresholdValue)
        vadThresholdValue.textContent = data.config.vad_threshold;
    }
  ...
  switch (data.type) {
    ...
    case "audio_chunk":
      ...
      if (Array.isArray(data.audio)) {
        console.log(
          `Received audio chunk: ${data.audio.length} samples at ${data.sample_rate}Hz`
        );
    ...
    case "status":
      ...
      if (
        data.message === "Models initialized" ||
        data.message === "Models initialized and configuration saved"
      ) {
        modelStatus.textContent = "Loaded";
        modelStatus.className = "font-medium text-green-500";
...
function addMessageToConversation(sender, text) {
  ...
  messageElement.className = `p-4 mb-4 rounded-lg ${
    sender === "user" ? "bg-gray-700 ml-12" : "bg-indigo-800 mr-12"
  }`;
    ...
    if (ws && ws.readyState === WebSocket.OPEN) {
      const config = {
        ...
        reference_audio_path:
          document.getElementById("referenceAudioPath").value,
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    window.audioContext = new (window.AudioContext ||
      window.webkitAudioContext)();
    console.log(
      "Audio context created on page load:",
  ...
  if (saveAudioSettingsBtn) {
    saveAudioSettingsBtn.addEventListener("click", () => {
      ...
      const settings = {
        ...
        speaker_volume: parseFloat(
          document.getElementById("speakerVolume").value
        ),
      ...
      if (ws && ws.readyState === WebSocket.OPEN) {
        ...
        showNotification(
          "Cannot save settings: not connected to server",
          "error"
        );
      }
  ...
  if (vadEnabled && micStatus) {
    vadEnabled.addEventListener("change", () => {
      micStatus.textContent = vadEnabled.checked
...
const startRecording = async (stream) => {
  ...
  try {
    ...
    const updateAudioLevels = () => {
      ...
      const newData = [
        ...audioLevelsChart.data.datasets[0].data.slice(1),
        average,
      ];
    ...
    processorNode.onaudioprocess = (e) => {
      ...
      console.log(
        `Sending audio chunk: length=${audioSlice.length}, sample_rate=${audioContext.sampleRate}`
      );
